...
export async function deleteFlair(subredditId: string, flairKey: string) {
  ...
  if (!isAdmin) throw new Error("Only admins can delete flairs");

  const result = await client
    .patch(subredditId)
    .unset([`allowedFlairs[_key == "${flairKey}"]`])
    .commit();
