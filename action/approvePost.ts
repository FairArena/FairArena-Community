...
export async function approvePost(postId: string, subredditId: string) {
  ...
  if (!isMod) throw new Error("Only moderators can approve posts");
  ...
  const result = await client
    ...
    .set({
      ...
      approvedBy: { _ref: user.id, _type: "reference" },
    })
    .commit();
