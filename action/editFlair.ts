...
export async function editFlair(
  subredditId: string,
  flairKey: string,
  flairName: string,
  flairColor: string
  ...
  if (!isAdmin) throw new Error("Only admins can edit flairs");

  const result = await client
    .patch(subredditId)
    .set({
      [`allowedFlairs[_key == "${flairKey}"].name`]: flairName,
      [`allowedFlairs[_key == "${flairKey}"].color`]: flairColor,
    })
    .commit();
